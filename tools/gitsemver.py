...
def getVersion(git: str = "git") -> Version:
  ...
  try:
    ...
    gitSHA = subprocess.check_output(cmd,
                                     universal_newlines=True,
                                     stderr=subprocess.DEVNULL).strip()
...
def checkInstallations(git: str = None,
                       gitConfig: bool = False,
  ...
  if git:
    ...
    if gitConfig:
      ...
      try:
        subprocess.check_call([git, "config", "--global", "user.name"],
                              cwd=".",
...
def main() -> None:
  ...
  parser.add_argument(
      ...
      metavar="FORMAT",
      help=
      "output version to stdout using the format: %%M major, %%m minor, %%p patch, %%t tweak, %%a ahead, %%~ modified, %%s SHA"
  )
  ...
  checkInstallations(git=args.git, quiet=args.quiet)
